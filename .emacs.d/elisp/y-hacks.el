...
(defun y-bury-and-delete ()
  "Bury the current buffer and delete the current window.
Testing as replacement for y-kill-buffer-and-window."
  (interactive)
...
(defun goto-line-with-feedback ()
  ...
  (interactive)
  (let ((y--linum-status (unless display-line-numbers-mode -1)))
    (unwind-protect
	(progn
	  (display-line-numbers-mode 1)
	  (call-interactively 'goto-line))
      (display-line-numbers-mode y--linum-status))))
...
(defun y-open (file)
  "Ask the system to open FILE."
  (interactive "f")
  (shell-command (concat
		  (pcase system-type
		    (`gnu/linux "xdg-open")
		    (`darwin "open")
		    (_ (read-shell-command "Open file with: ")))
		  " "
		  (expand-file-name file))))

;; Add a dired command to open the file
(eval-after-load "dired"
  '(progn
     (define-key dired-mode-map (kbd "z")
       (lambda ()
	 (interactive)
	 (y-open (dired-get-file-for-visit))))))
